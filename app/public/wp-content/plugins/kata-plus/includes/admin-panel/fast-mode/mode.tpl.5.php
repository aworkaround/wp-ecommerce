...
<div id="kt-fst-mod-5" class="kt-fst-mod-wrapper kata-admin">
	<h1 id="page-title" class="chose-bussiness-type"><?php echo esc_html__( 'Select your favorite design for start the import', 'kata-plus-pro' ); ?></h1>

	<div class="fast-mode-search-demo search">
		<div class="fm-search">
			<?php echo wp_kses_post( Kata_Plus_Helpers::get_icon( '', 'font-awesome/search', '', '' ) ); ?>
		</div>
		<input type="text" name="seach" class="site-search" placeholder="<?php echo esc_html__( 'search', 'kata-plus-pro' ); ?>" value="">
	</div>
					...
					*/
					foreach ( $demos as $key => $value ) {
						...
						if ( ! $is_hidden ) {
							...
							$imported_demo = isset( $kata_options['imported_demos'] ) && in_array( $key, $kata_options['imported_demos'] ) ? esc_html__( 'This demo already imported', 'kata-plus-pro' ) : '';
							?>
							<div class="kata-col-sm-4 kata-importer" data-wishlist="false" website-type="all <?php echo esc_attr( $websitetype ); ?>" demo-name="<?php echo esc_attr( strtolower( $value->name ) ); ?>" import-status="<?php echo esc_attr( $imported_demo ); ?>" demo-screenshot="<?php echo esc_attr( $image ); ?>" data-key="<?php echo esc_attr( $key ); ?>" demo-type="<?php echo esc_attr( $is_pro ); ?>">
								<div class="kata-demp-wrapper">
									...
									<a class="kata-importer-preview" href="<?php echo esc_url( @$value->preview_url ); ?>" target="_blank"><?php echo esc_attr__( 'Preview', 'kata-plus-pro' ); ?></a>
									<h3 class="kata-importer-name">
										<span class="demo-type <?php echo esc_attr( $is_pro ); ?>"><?php echo esc_html( $is_pro ); ?></span>
										<?php echo esc_html( $value->name ); ?>
									</h3>
									<button class="kata-btn kata-btn-importer"><?php echo esc_html__( 'Import', 'kata-plus-pro' ); ?></button>
									<button class="kata-btn kata-btn-xd" title="<?php echo esc_attr__( 'Download .XD File', 'kata-plus-pro' ); ?>"><?php echo esc_html__( '.XD', 'kata-plus-pro' ); ?></button>
					...
					?>
					<p><?php echo esc_html__( 'Oops! There are no demos available', 'kata-plus-pro' ); ?></p>
					<?php
				}
				?>
			</div> <!-- end .row -->
	...
	<a href="#" class="fast-mode-loadmore"><?php echo esc_html__( 'Load more...', 'kata-plus-pro' ); ?></a>
</div>
<div class="kt-fst-mod-footer-area kt-fst-mod-5">
	<a href="<?php echo esc_url( admin_url( 'admin.php?page=kata-plus-fast-mode&step=4' ) ); ?>" class="prev-step">
		...
		<?php echo esc_html__( 'Back', 'kata-plus-pro' ); ?>
	</a>
