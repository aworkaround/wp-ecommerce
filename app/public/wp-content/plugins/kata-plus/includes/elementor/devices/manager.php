...
class Manager extends Module {
	...
	const BREAKPOINT_KEY_SMALLMOBILE     = 'smallmobile';
	const BREAKPOINT_KEY_TABLET          = 'tablet';
	const BREAKPOINT_KEY_TABLET_EXTRA    = 'tablet_extra';
	const BREAKPOINT_KEY_TABLETLANDSCAPE = 'tabletlandscape';
	...
	public function get_responsive_icons_classes_map( $device = null ) {
		if ( ! $this->icons_map ) {
			$this->icons_map = array(
				...
				'smallmobile'     => 'eicon-device-mobile small',
				'mobile_extra'    => 'eicon-device-mobile eicon-tilted',
				'tablet'          => 'eicon-device-tablet',
				'tabletlandscape' => 'eicon-device-tablet eicon-tilted',
	...
	public static function get_default_config() {
		return array(
			self::BREAKPOINT_KEY_SMALLMOBILE     => array(
				'label'         => esc_html__( 'Small Mobile', 'elementor' ),
				'default_value' => 320,
				'direction'     => 'max',
			),
			...
			self::BREAKPOINT_KEY_TABLETLANDSCAPE => array(
				'label'         => esc_html__( 'Tablet Landscape', 'elementor' ),
				'default_value' => 1024,
				'direction'     => 'max',
			),
			self::BREAKPOINT_KEY_TABLET_EXTRA    => array(
				'label'         => esc_html__( 'Extera Tablet', 'elementor' ),
				'default_value' => 1080,
				'direction'     => 'max',
			),
	...
	private function init_breakpoints() {
		...
		$active_breakpoint_keys = array(
			$setting_prefix . self::BREAKPOINT_KEY_SMALLMOBILE,
			$setting_prefix . self::BREAKPOINT_KEY_MOBILE,
			$setting_prefix . self::BREAKPOINT_KEY_TABLET,
			$setting_prefix . self::BREAKPOINT_KEY_TABLETLANDSCAPE,
			$setting_prefix . self::BREAKPOINT_KEY_LAPTOP,
		);
	...
	private function init_active_breakpoints() {
		$this->active_breakpoints = array_filter(
	...
	private function get_desktop_previous_device_key() {
		...
		if ( self::BREAKPOINT_KEY_WIDESCREEN === $config_array_keys[ $num_of_devices - 1 ] ) {
			...
			$desktop_previous_device = $config_array_keys[ $num_of_devices ];
		}
